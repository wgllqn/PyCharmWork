...
export function DataSourceManager({
  ...
  const loadDataSources = async () => {
    try {
      ...
      console.log('数据源响应:', response); // 调试用
      if (response.success) {
  ...
  const groupedSources = {
    database: dataSources.filter(s => s.source === 'database'),  // 外部真实数据库
    mock: dataSources.filter(s => s.source === 'mock' || s.source === 'derived'),  // 模拟数据
    upload: dataSources.filter(s => s.source === 'upload')  // 上传文件
  };
      ...
      <CardContent className="pt-0">
        <ScrollArea className="h-[600px]">
          <div className="space-y-4">
            {/* 外部数据库 */}
            {groupedSources.database.length > 0 && (
              <div>
                <Button
                  variant="ghost"
            ...
            {/* 模拟数据源 */}
            {groupedSources.mock.length > 0 && (
              <div>
                <Button
                  variant="ghost"
                  className="w-full justify-between p-2"
                  onClick={() => toggleExpand('mock')}
                >
                  <span className="flex items-center gap-2">
                  ...
                  <ChevronRight className={`w-4 h-4 transition-transform ${
                    expanded.includes('mock') ? 'rotate-90' : ''
                  }`} />
                </Button>

                {expanded.includes('mock') && (
                  <div className="ml-4 mt-2 space-y-2">
                    {groupedSources.mock.map((source) => (
                    <div
                      key={source.table}
                      className={`p-3 rounded-lg border cursor-pointer transition-colors ${
                        selectedSource?.table === source.table
